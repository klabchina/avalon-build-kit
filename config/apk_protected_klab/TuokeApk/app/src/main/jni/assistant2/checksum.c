...
bool fetch_checksum_of_library_signal_file(const char* filePath, textSection* pTextSection){
    ...
    read(fd, &ehdr, ehdr_size);
    lseek(fd, (off_t)ehdr.e_shoff, SEEK_SET);

    for(int i = 0; i < ehdr.e_shnum; i++){
        memset(&sectHdr,0,shdr_size);
        read(fd, &sectHdr, shdr_size);
...
void scan_sum_exec_segments(char* map, textSection* pElfSectArr, int idx){
    ...
    if (buf[2] == 'x' && buf[0] == 'r') {
        ...
        if (idx > 0 && memsize >= pElfSectArr->memsize)
    ...
    if ((fd = my_openat(AT_FDCWD, "/proc/self/maps", O_RDONLY | O_CLOEXEC, 0 )) != 0) {
        ...
        while ((read_one_line(fd, map, MAX_LINE)) > 0) {
            for(int i = 0; i < NUM_LIBS; i++) {
                ...
                if (strstr(map, lib_names) != NULL) {
                    scan_sum_exec_segments(map, elfSectionArr[i], i);
                }
            }
        }
...
void record_sum()
{
    ...
    for(int i = 0; i < NUM_LIBS; i++)
    {
        fetch_checksum_of_library(filePaths[i], &elfSectionArr[i], i);
        if (filePaths[i] != "")
        {
            free(filePaths[i]);
        }
