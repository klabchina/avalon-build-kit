	...
	protected void attachBaseContext(Context base) {
		...
		try {
			...
			if (dexFile.length() == 0)
			{
				...
				Log.i("demo", "after wrtie apk size:" + dexFile.length());
				// 分离出解壳后的apk文件已用于动态加载
			...
			Log.i("demo","Libs Path:" + this.getApplicationInfo().nativeLibraryDir);
			
			//创建被加壳apk的DexClassLoader对象  加载apk内的类和本地代码（c/c++代码）
			DexClassLoader dLoader = new DexClassLoader(apkFileName, odexPath,
					this.getApplicationInfo().nativeLibraryDir, (ClassLoader) RefInvoke.getFieldOjbect(
...
//			try{
//				Object actObj = dLoader.loadClass("com.avalon.sdk.test.MainActivity");
...
//			}
	...
	private void splitPayLoadFromDex(byte[] apkdata) throws IOException {
		...
		Log.i("demo","fileReWritelENGTH:" + file.length());
	...
	protected void loadResources(String dexPath) {  
        try {  
        	...
        	Log.i("assetManager",  assetManager.toString());
